...
- `INC-2025-1123-LOG-R7` - Added debug logs to `EnemyAISubsystem::ComputeMoveOrWaitIntent` to trace why primary moves are rejected and alternates chosen (`AI/Enemy/EnemyAISubsystem.cpp`) (2025-11-23 03:00)
- `INC-2025-1123-FIX-R1` - Updated `EnemyAISubsystem::ComputeMoveOrWaitIntent` to use DistanceField cost (if available) instead of Chebyshev distance for primary move validation, allowing enemies to take paths that temporarily increase Chebyshev distance to navigate around terrain (`AI/Enemy/EnemyAISubsystem.cpp`) (2025-11-23 03:30)
- `INC-2025-1123-FIX-R2` - Relaxed `DistanceFieldSubsystem::CanMoveDiagonal` rule to allow diagonal movement if at least one orthogonal shoulder is walkable, preventing excessive detours around single corner obstacles (`Turn/DistanceFieldSubsystem.cpp`) (2025-11-23 03:45)
- `INC-2025-1123-FIX-R3` - Removed automatic rerouting logic from `ConflictResolverSubsystem::ResolveAllConflicts` (Phases B and C). This prevents the resolver from overriding AI decisions with suboptimal moves (like sideways shuffling) when the intended move is blocked. Now, blocked moves simply result in a WAIT, allowing the AI to re-evaluate in the next turn (`Turn/ConflictResolverSubsystem.cpp`) (2025-11-23 04:00)
- `INC-2025-1123-FIX-R4` - Disabled `INC-2025-1158-R1` (Equal Distance Wait logic) in `EnemyAISubsystem::ComputeMoveOrWaitIntent`. This allows enemies to take alternate moves that are equidistant to the player (e.g., following a leader) instead of waiting when their primary path is blocked by another unit. This resolves issues where enemies would get stuck behind each other (`AI/Enemy/EnemyAISubsystem.cpp`) (2025-11-23 04:30)
- `INC-2025-1123-FIX-R5` - Updated `ConflictResolverSubsystem::ResolveAllConflicts` (Phase A-2) to check for external reservations (e.g., Player moving via GAS) when identifying stationary blockers. If an actor has a reservation, it now blocks its *destination* instead of its *current cell*. This fixes the issue where enemies would treat the player's previous position as blocked during simultaneous turns, preventing them from chasing effectively (`Turn/ConflictResolverSubsystem.cpp`) (2025-11-23 05:30)
- `INC-2025-1123-FIX-R6` - Added `IsAttackLineClear` to `EnemyTurnDataSubsystem` and integrated it into `UpgradeIntentsForAdjacency`. This prevents enemies from upgrading their intent to ATTACK when the line of sight to the player's target position is blocked by a corner (corner cutting), ensuring valid diagonal attacks (`AI/Enemy/EnemyTurnDataSubsystem.cpp`) (2025-11-23 06:00)
